...
class TasksWorkspace(BaseWorkspace):
        ...
        *,
        ...
        try:
            created = self.model.add_task(
                ...
                board_column=values.get("board_column", BOARD_COLUMN_QUEUE),
    ...
    def eventFilter(self, obj, event) -> bool:
        ...
        if obj is self.list.viewport() and event.type() == QEvent.Type.Resize:
            self._update_sticky_day_header()
            self._update_batch_bar_geometry()
        if obj is self.list.viewport() and event.type() == QEvent.Type.MouseMove:
            if isinstance(event, QMouseEvent):
                index = self.list.indexAt(event.position().toPoint())
                current_row = index.row() if index.isValid() else None
                if self._project_type_hover_row is not None and self._project_type_hover_row != current_row:
                    previous_index = self.model.index(self._project_type_hover_row, 0)
                    if previous_index.isValid():
                        self.list.viewport().update(self.list.visualRect(previous_index))
                self._project_type_hover_row = current_row
                if index.isValid():
                    self.list.viewport().update(self.list.visualRect(index))
        if obj is self.list.viewport() and event.type() == QEvent.Type.Leave:
            if self._project_type_hover_row is not None:
                previous_index = self.model.index(self._project_type_hover_row, 0)
                if previous_index.isValid():
                    self.list.viewport().update(self.list.visualRect(previous_index))
            self._project_type_hover_row = None
